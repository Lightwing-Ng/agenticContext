...
def test_browser_acceptance_real_chromium_desktop_and_narrow_cleanup(tmp_path: Path) -> None:
    ...
    (root / "index.html").write_text(
        "<!doctype html><html><body><main>Loading</main><script>document.querySelector('main').textContent = document.cookie ? 'Cookie leaked' : 'Ready';</script></body></html>",
        encoding="utf-8",
    )
    observed = []
...
def test_browser_acceptance_missing_target_and_external_resource_fail_closed(tmp_path: Path) -> None:
    playwright_sync = pytest.importorskip("playwright.sync_api")
    root = tmp_path / "site"
    root.mkdir()
    (root / "index.html").write_text(
        '<!doctype html><html><body><main>Ready</main><img src="https://example.invalid/pixel.png"></body></html>',
        encoding="utf-8",
    )
    blocked = run_browser_acceptance(
        _request(tmp_path, root),
        playwright_context=playwright_sync.sync_playwright,
        process_group_options=_process_group_options,
        stop_process=_stop_process,
    ...
    assert all(entry["blocked_requests"] for entry in blocked["viewports"])
    assert all(
        all(value.startswith("sha256:") for value in entry["blocked_requests"])
        for entry in blocked["viewports"]
    )

    missing = run_browser_acceptance(
        _request(tmp_path, root, target="/missing.html", expected_text=(), expected_selectors=()),
        playwright_context=playwright_sync.sync_playwright,
        process_group_options=_process_group_options,
        stop_process=_stop_process,
        process_changed=lambda _process: None,
    ...
    assert all(entry["http_status"] == 404 for entry in missing["viewports"])
