...
from app.core.computer_use_agent import (
    ...
    _grok_fresh_navigation_allowed,
    _web_target_is_open,
...
def test_settings_validate_all_web_agent_platforms_and_model_contracts() -> None:
    ...
    assert AGENT_MODEL_OPTIONS_BY_PLATFORM["grok"][1]["ui_label"] == "Auto"
    assert AGENT_MODEL_OPTIONS_BY_PLATFORM["grok"][1]["remote_labels"] == ("Auto",)
    ...
    with TemporaryDirectory() as raw_root:
        ...
        for safari_platform, safari_model, safari_target in (
            ...
            safari_sources = validate_computer_use_settings(
                {
                    "workspace_path": raw_root,
                    "platform": safari_platform,
                    "browser": "safari",
                    "model": safari_model,
                    "target_url": safari_target,
                }
            )
...
def test_current_grok_auto_setting_is_preserved(tmp_path: Path) -> None:
    settings_path = tmp_path / "computer-use-agent.json"
    workspace = tmp_path / "project"
    workspace.mkdir()
    settings_path.write_text(
    ...
    assert settings.platform == "grok"
    assert settings.model == "grok-auto"
    assert json.loads(settings_path.read_text(encoding="utf-8"))["model"] == "grok-auto"
...
@pytest.mark.integration
def test_safari_chatgpt_selector_accepts_select_model_and_verifies_extra_high(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Exercise the current combined ChatGPT menu through the Safari selector path."""
    import app.core.computer_use_agent as computer_use_agent
        ...
        if Path(browser_type.executable_path).is_file():
            ...
            for channel in ("chrome", "msedge"):
                try:
                    browser = browser_type.launch(channel=channel, headless=True)
                    break
                except Exception:  # pragma: no cover - host browser inventory
                    continue
            if browser is None:
                pytest.skip("A local Chromium, Chrome, or Edge executable is unavailable.")
        try:
            page = browser.new_page()
            page.set_content(
                """
                <style>
                  [role="slider"], [role="menuitem"], [role="menuitemradio"] {
                    display: block;
                    width: 120px;
                    height: 20px;
                  }
                ...
                <form data-type="unified-composer">
                  <textarea id="prompt-textarea"></textarea>
                  <button id="power" type="button" class="__composer-pill" aria-haspopup="menu"
                    aria-expanded="false" aria-controls="power-menu">Medium</button>
                  <div id="power-menu" role="menu" hidden>
                    <div data-model-selection-view="true">
                      <div id="simple" data-testid="composer-model-picker-slider-simple-view">
                        <div id="select-model" role="menuitem" tabindex="0"
                          aria-label="Select model">Select model</div>
                        <div data-model-reasoning-effort-slider aria-label="Thinking effort">
                          <div id="effort" role="slider" tabindex="0" aria-valuemin="0"
                            aria-valuemax="3" aria-valuenow="1" aria-valuetext="Medium"></div>
                        </div>
                      </div>
                      <div id="advanced" data-testid="composer-model-picker-slider-advanced-view"
                        inert>
                        <div role="menuitemradio" tabindex="0" aria-checked="true">Latest</div>
                        <div role="menuitemradio" tabindex="0" aria-checked="false">GPT-5.6 Sol</div>
                        <div role="menuitemradio" tabindex="0" aria-checked="false">GPT-5.5</div>
                      </div>
                    </div>
                  </div>
                </form>
                <script>
                  (() => {
                    ...
                    let selectModelClicks = 0;
                    const setModelView = (open) => {
                      simple.toggleAttribute('inert', open);
                      advanced.toggleAttribute('inert', !open);
                    };
                    power.addEventListener('click', () => {
                      const open = power.getAttribute('aria-expanded') === 'true';
                      power.setAttribute('aria-expanded', String(!open));
                      menu.hidden = open;
                      if (!open) setModelView(false);
                    });
                    document.querySelector('#select-model').addEventListener(
                      'click', () => {
                        selectModelClicks += 1;
                        if (selectModelClicks === 1) {
                    ...
                    choices.forEach((choice) => choice.addEventListener('click', () => {
                      choices.forEach((candidate) => candidate.setAttribute(
                        'aria-checked', String(candidate === choice)
                      ));
                      setModelView(false);
                    }));
                    slider.addEventListener('keydown', (event) => {
                      ...
                      if (event.key === 'ArrowRight') value = Math.min(value + 1, 3);
            ...
            bound_slider, bound_scope = _chatgpt_effort_slider_binding(
                page,
                trusted_model_menu_scope="menu:power-menu",
            )
            assert bound_slider is not None
            assert bound_scope == "menu:power-menu"
            page.locator("#power").click()
            observation: dict[str, object] = {}
            original_click = computer_use_agent._chatgpt_safari_click
            click_attempts = 0

            def click_after_safe_transients(
                *args: object,
                **kwargs: object,
            ) -> tuple[bool, str]:
            ...
            monkeypatch.setattr(
                computer_use_agent,
                "_chatgpt_safari_click",
                click_after_safe_transients,
            )
            ...
            ) is True, json.dumps(observation, sort_keys=True)
            assert observation["observed"] == "Latest"
            assert observation["thinking_effort"] == "Extra High"
            assert observation["available_efforts"] == [
                "Instant",
                "Medium",
                "High",
            ...
            assert page.locator("#power-menu").is_hidden()
        finally:
            browser.close()


def test_safari_chatgpt_click_defers_attempted_native_input_to_readback(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import app.core.computer_use_agent as computer_use_agent
    from app.core.safari_automation import SafariContext, SafariLocator, SafariPage

    page = SafariPage(SafariContext("https://chatgpt.com/"), window_id=123)
    monkeypatch.setattr(SafariLocator, "count", lambda _self: 1)
    monkeypatch.setattr(SafariLocator, "is_visible", lambda _self: True)

    def uncertain_click(*_args: object, **_kwargs: object) -> None:
        raise SafariNativeActivationError(
            "The DOM changed after native input.",
    ...
    clicked, reason = computer_use_agent._chatgpt_safari_click(
        page,
        "data-test-marker",
        "marker-1",
        "https://chatgpt.com/",
    ...
    assert reason == "model-selection-click-uncertain"


def test_chatgpt_effort_key_defers_attempted_safari_input_to_readback() -> None:
    import app.core.computer_use_agent as computer_use_agent

    class _Slider:
        def press(self, _key: str, **_kwargs: object) -> None:
            raise SafariNativeActivationError(
                "The slider remounted after native input.",
                input_attempted=True,
            )

    assert computer_use_agent._chatgpt_press_effort_key(
        object(),
        _Slider(),
        "ArrowRight",
        scope="menu:power-menu",
    ) is True


def test_chatgpt_effort_key_rebinds_after_blocked_safari_input(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import app.core.computer_use_agent as computer_use_agent
    ...
    class _Slider:
        ...
        def press(self, _key: str, **_kwargs: object) -> None:
            ...
            if self.presses == 1:
                raise SafariNativeActivationError(
                    "Safari did not send the native key.",
    ...
    assert slider.presses == 2


def test_chatgpt_effort_label_accepts_provider_text_without_sentence_spacing() -> None:
    import app.core.computer_use_agent as computer_use_agent

    assert computer_use_agent._chatgpt_normalize_effort_label(
        "Extra High, 4 of 4.Use Left and Right arrow keys to adjust power."
    ) == "Extra High"


def test_chatgpt_effort_label_defers_a_safari_locator_remount() -> None:
    class _Slider:
        ...
        def evaluate(self, _expression: str) -> object:
            raise RuntimeError(
                "Safari JavaScript failed: Safari locator element is unavailable."
            )

    assert _chatgpt_slider_effort_label(_Slider()) == ""
...
def test_safari_gemini_model_selection_uses_synchronous_native_readback() -> None:
    class _Locator:
        def __init__(self, page: "_Page", kind: str) -> None:
            self.page = page
            self.kind = kind
            self.first = self
        ...
        def click(self, **_kwargs: object) -> None:
            if self.kind == "trigger":
                self.page.trigger_attempts += 1
                if self.page.trigger_attempts == 1:
                    raise SafariNativeActivationError(
                        "Safari did not send the native key.",
                        input_attempted=False,
                    )
    ...
    class _Page:
        ...
        def evaluate(
            ...
            assert "async" not in expression
            assert arguments["platform"] == "gemini"
            ...
            return {
                ...
                "current": "Flash",
                "targetCount": 1,
                "targetSelected": self.selected,
                "selected": "3.1 Pro" if self.selected else "",
                "selectedCount": 1 if self.selected else 0,
                "available": ["3.8 Flash", "3.1 Pro"],
            }

        def locator(self, selector: str) -> _Locator:
            return _Locator(
                self,
                "choice" if "web-model-choice" in selector else "trigger",
            )
    ...
    assert page.trigger_attempts == 4
    assert page.trigger_clicks == 3
    assert page.choice_clicks == 1
    assert page.expanded is False
    assert observation["observed"] == "3.1 Pro"
    assert observation["available"] == ["3.8 Flash", "3.1 Pro"]
...
@pytest.mark.parametrize(
    ("input_attempted", "expected_reason"),
    (
        (True, "model-selection-click-uncertain"),
        (False, "model-selection-native-input-blocked"),
    ),
)
def test_safari_grok_model_trigger_does_not_repeat_failed_native_input(
    monkeypatch: pytest.MonkeyPatch,
    ...
    class _Locator:
        ...
        def click(self, **_kwargs: object) -> None:
            self.clicks += 1
            raise SafariNativeActivationError(
                "Native input transport became uncertain.",
    ...
    class _Page:
        ...
        def __init__(self) -> None:
            self.trigger = _Locator()
            self.waits: list[int] = []

        def locator(self, selector: str) -> _Locator:
            assert "data-cachelikes-grok-model-trigger" in selector
            return self.trigger
    ...
    monkeypatch.setattr(
        computer_use_agent,
        "_dismiss_known_grok_onboarding_dialogs",
        lambda *_args, **_kwargs: (True, ""),
    )
    monkeypatch.setattr(
        computer_use_agent,
        "_grok_blocking_dialog_snapshot",
        lambda _page: {"blocking": False},
    )
    monkeypatch.setattr(
        computer_use_agent,
        "_grok_model_trigger_snapshot",
        lambda *_args: {
            "ok": True,
            "current": "Auto",
            "expanded": False,
            "controlledId": "model-menu",
        },
    )
    monkeypatch.setattr(
        computer_use_agent,
        "_inspect_open_grok_model_surface",
        lambda *_args: {"ok": False, "reason": "model-surface-not-found"},
    )
    observation: dict[str, object] = {}

    assert computer_use_agent._select_grok_model_with_trusted_clicks(
        page,
        "safari",
        ("Build",),
        ("Build",),
...
@pytest.mark.parametrize(
    ("model", "expected_labels", "expected_trigger_labels"),
    (
        ("grok-build", ("Build",), ("Build Beta",)),
        ("grok-auto", ("Auto",), ("Auto",)),
    ),
)
def test_safari_grok_model_selection_restores_each_native_input_independently(
    ...
    expected_trigger_labels: tuple[str, ...],
) -> None:
    import app.core.computer_use_agent as computer_use_agent
    ...
    def select(
        ...
        **_kwargs: object,
        ...
        events.append(
            f"select:{browser_kind}:{remote_labels!r}:{trigger_labels!r}"
        )
        return True
...
def test_open_browser_for_login_macos_chrome_keeps_standard_handoff(
    ...
    """macOS Chrome login stays on the daily-browser handoff."""
    from unittest.mock import Mock

    import app.core.computer_use_agent as computer_use_agent

    expected = {"opened": True, "browser": "chrome"}
    handoff = Mock(return_value=expected)
    config = CrawlConfig()
    monkeypatch.setattr(computer_use_agent.sys, "platform", "darwin")
    monkeypatch.setattr(computer_use_agent, "is_windows_host", lambda: False)
    ...
    result = open_browser_for_login("chatgpt", "chrome", config=config)
...
def test_open_browser_for_login_macos_allows_safari_source_providers(
    monkeypatch: pytest.MonkeyPatch,
    platform: str,
    home_url: str,
) -> None:
    ...
    expected = {"opened": True, "browser": "safari", "platform": platform}
    handoff = Mock(return_value=expected)
    config = CrawlConfig()
    monkeypatch.setattr(computer_use_agent.sys, "platform", "darwin")
    monkeypatch.setattr(computer_use_agent, "is_windows_host", lambda: False)
...
def test_grok_target_check_rejects_untrusted_origins(untrusted_url: str) -> None:
    assert not _web_target_is_open("grok", "https://grok.com/", untrusted_url)
    assert not _web_target_is_open("grok", untrusted_url, "https://grok.com/")
    assert not _grok_fresh_navigation_allowed(untrusted_url, "https://grok.com/c/session")


def test_grok_target_check_accepts_the_explicit_standard_https_port() -> None:
    assert _web_target_is_open(
        "grok",
        "https://grok.com:443/c/session",
        "https://www.grok.com:443/c/session",
    )
...
def test_settings_preference_updates_hold_one_read_modify_write_lock(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import app.core.computer_use_agent as computer_use_agent
    ...
    original_validate = computer_use_agent.validate_computer_use_settings

    def delayed_validate(payload: dict[str, object]) -> ComputerUseSettings:
        if payload.get("platform") == "grok":
    ...
    first = Thread(
        target=lambda: store.update_preferences(
            workspace_path=str(workspace),
            operating_system="macos",
            platform="grok",
            browser="safari",
    ...
    second = Thread(target=save_second)
    first.start()
    assert first_validating.wait(timeout=2)
    second.start()
    try:
    ...
    assert store.settings.platform == "gemini"
    assert store.settings.browser == "edge"


def test_settings_preference_revision_rejects_a_late_stale_page_write(
    tmp_path: Path,
) -> None:
    workspace = tmp_path / "project"
    workspace.mkdir()
    ...
    latest = store.update_preferences(
        workspace_path=str(workspace),
        operating_system="macos",
        platform="grok",
        browser="safari",
    ...
    stale = store.update_preferences(
        workspace_path=str(workspace),
        operating_system="macos",
        platform="gemini",
        browser="edge",
    ...
    assert latest.platform == "grok"
    assert latest.browser == "safari"
    assert stale == latest
    assert store.settings == latest
...
def test_stop_during_browser_startup_never_enters_the_action_loop(
    ...
    monkeypatch.setattr(
        computer_use_agent,
        "SafariContext",
        lambda _target_url, *, lock_blocking: browser_context,
    )
    monkeypatch.setattr(
        computer_use_agent,
        "sync_playwright_or_error",
...
def test_fresh_gemini_binding_latches_app_id_url_without_a_visible_receipt() -> None:
    created_url = "https://gemini.google.com/app/683bed8574e85719"
    ...
    binding.arm_first_submission("Inspect the claim")
    page.url = created_url
...
def test_safari_submission_uses_one_native_send_activation() -> None:
    class _Page:
        url = "https://chatgpt.com/"
        ...
        def evaluate(self, expression: str, argument: object = None) -> dict[str, object]:
            ...
            return {"ready": True, "ariaLabel": "Send prompt", "dataTestId": "send-button"}
    ...
    assert any("data-cachelikes-safari-send" in expression for expression in expressions)
    assert all("sendButton.click()" not in expression for expression in expressions)
    assert all("KeyboardEvent" not in expression for expression in expressions)
    assert page.native_clicks == 1
...
def test_safari_grok_submission_uses_the_unique_provider_composer_and_send_scope() -> None:
    class _Page:
        ...
        def __init__(self) -> None:
            self.evaluate_calls: list[tuple[str, object]] = []
            self.native_clicks = 0
            self.native_click_kwargs: dict[str, object] = {}
        ...
        def locator(self, selector: str) -> object:
            ...
            class _Locator:
                ...
                def click(self, **_kwargs: object) -> None:
                    page.native_clicks += 1
                    page.native_click_kwargs = dict(_kwargs)
        ...
        def evaluate(self, expression: str, argument: object = None) -> dict[str, object]:
            ...
            return {"ready": True, "ariaLabel": "Submit", "dataTestId": "chat-submit"}
    ...
    assert "sendButton.click()" not in page.evaluate_calls[1][0]
    assert page.native_clicks == 1
...
def test_safari_submission_refills_when_the_composer_readback_drifts() -> None:
    class _Page:
        url = "https://chatgpt.com/"
        ...
        def evaluate(self, expression: str, _argument: object = None) -> dict[str, object]:
            ...
            if "composer.focus()" in expression:
                ...
                return {"filled": True, "composerCount": 1}
            if self._fills < 2:
                return {"clicked": False, "composerCount": 1, "composerExact": False}
            return {"ready": True, "ariaLabel": "Send prompt", "dataTestId": "send-button"}
    ...
    assert page._fills == 2
    assert page.native_clicks == 1
    assert any(
        "element.tagName === 'TEXTAREA' || element.tagName === 'INPUT'" in expression
        for expression in page.evaluate_calls
    )
    assert any(
        "agent-(?:transfer|turn)-[0-9a-f]{16,}" in expression
        for expression in page.evaluate_calls
    )


def test_safari_grok_submission_never_repeats_uncertain_native_send() -> None:
    class _Page:
        url = "https://grok.com/"
        ...
        def wait_for_timeout(self, _milliseconds: int) -> None:
            raise AssertionError("An uncertain Send must not enter a retry loop.")
...
def test_safari_stop_rejects_ambiguous_controls_before_native_input() -> None:
    import app.core.computer_use_agent as computer_use_agent

    class _Page:
        url = "https://grok.com/c/session"

        def __init__(self) -> None:
            self.locator_calls = 0

        def evaluate(self, expression: str, _argument: object = None) -> object:
            if "expectedCurrentUrl" in expression:
                return {"count": 2}
            return None

        def locator(self, _selector: str) -> object:
            self.locator_calls += 1
            raise AssertionError("Ambiguous Stop controls must not reach native input.")

    page = _Page()
    computer_use_agent._stop_web_generation(page, "safari")
    assert page.locator_calls == 0


def test_safari_stop_does_not_repeat_uncertain_native_input(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import app.core.computer_use_agent as computer_use_agent
    ...
    class _Page:
        ...
        def locator(self, selector: str) -> _Locator:
            assert "data-cachelikes-safari-stop" in selector
            return self.stop
    ...
    monkeypatch.setattr(
        computer_use_agent,
        "_web_is_generating",
        lambda *_args: next(generating),
    )
    page = _Page()

    computer_use_agent._stop_web_generation(page, "safari")

    assert page.stop.clicks == 1
    assert page.stop.kwargs == {
        "timeout": 3_000,
...
def test_safari_submission_rejects_ambiguous_enabled_send_buttons() -> None:
    class _Page:
        url = "https://grok.com/"
        ...
        def evaluate(self, expression: str, _argument: object = None) -> dict[str, object]:
            ...
            return {
                ...
                "enabledSendButtonCount": 2,
            }

        def locator(self, _selector: str) -> object:
            self.locator_calls += 1
            raise AssertionError("Ambiguous controls must fail before native input.")

        def wait_for_timeout(self, _milliseconds: int) -> None:
            raise AssertionError("Ambiguous controls must fail without polling.")

    page = _Page()
    with pytest.raises(RuntimeError, match="multiple enabled Grok send buttons"):
        _submit_safari_prompt(
            page,
            "Inspect safely.",
            lambda: False,
    ...
    assert page.locator_calls == 0
    assert "enabledSendButtons.length === 1" in page.evaluate_calls[1]
    assert "enabledSendButtons.length > 1" in page.evaluate_calls[1]
...
def test_safari_submission_never_clicks_send_after_stop(stop_stage: str) -> None:
        ...
        def evaluate(self, expression: str, _argument: object = None) -> dict[str, object]:
            ...
            if "composer.focus()" in expression:
                if stop_stage == "after_fill":
                    stop_requested.set()
                return {"filled": True, "tagName": "DIV", "contentEditable": True}
            assert "data-cachelikes-safari-send" in expression
    ...
    send_attempts = [
        expression
        for expression in page.evaluate_calls
        if "data-cachelikes-safari-send" in expression
        and "composer.focus()" not in expression
    ]
...
def test_grok_submission_does_not_press_enter_after_stop_during_button_scan() -> None:
    ...
    class _Page:
        def __init__(self) -> None:
            self.composer = _Composer()
            self.url = "https://grok.com/c/bound-session"
...
def test_challenge_marker_in_chat_text_does_not_pause_with_a_visible_composer() -> None:
    ...
    assert _provider_human_verification_reason(_Page(), "chatgpt") == ""
...
@pytest.mark.parametrize(
    ("platform", "reason"),
    (
        ("chatgpt", "security challenge control"),
...
def test_chatgpt_cloudflare_title_survives_dom_evaluation_failure() -> None:
    class _Page:
        def evaluate(self, _expression: str, _argument: object) -> dict[str, object]:
            raise RuntimeError("Cloudflare replaced the execution context")
    ...
    assert _provider_human_verification_reason(_Page(), "chatgpt") == (
        "Human verification required: ChatGPT requires Cloudflare security verification."
    )


def test_chatgpt_authorize_url_is_human_verification_without_evaluate() -> None:
    class _Page:
        url = "https://auth.openai.com/api/accounts/authorize?client_id=app_X&prompt=login"

        def evaluate(self, _expression: str, _argument: object) -> dict[str, object]:
            raise AssertionError("Authorize pages must not be evaluated")

        def title(self) -> str:
            raise AssertionError("Authorize pages must not be titled")

        def locator(self, _selector: str) -> object:
            raise AssertionError("Authorize pages must not be read")

        def content(self) -> str:
            raise AssertionError("Authorize pages must not be read")

    assert _provider_human_verification_reason(_Page(), "chatgpt") == (
        "Human verification required: ChatGPT requires sign-in or security verification."
    )


def test_chatgpt_composer_wait_does_not_reload_a_cloudflare_challenge(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import app.core.computer_use_agent as computer_use_agent
    ...
    class _Composer:
        ...
        def wait_for(self, **_kwargs: object) -> None:
            raise TimeoutError("composer stalled behind Cloudflare")
    ...
    with pytest.raises(RuntimeError, match="did not become ready after one reload"):
        computer_use_agent._wait_for_chromium_composer(
            page,
            expected_target_url="https://chatgpt.com/",
        )
...
def test_chatgpt_composer_wait_defers_same_origin_retry_mismatch(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import app.core.computer_use_agent as computer_use_agent
    ...
    class _Composer:
        def wait_for(self, **_kwargs: object) -> None:
            raise TimeoutError("composer still opening")
    ...
    class _Page:
        ...
        def evaluate(self, expression: str, _argument: object = None) -> dict[str, object]:
            if "retryButton.click()" in expression:
                raise AssertionError("A same-origin URL drift must not click Retry.")
            return {"detected": False, "reason": "", "composerAvailable": False}
    ...
    monkeypatch.setattr(computer_use_agent, "CHATGPT_COMPOSER_TIMEOUT_SECONDS", 0.6)
    monkeypatch.setattr(computer_use_agent, "WEB_SEND_BUTTON_POLL_MILLISECONDS", 50)
    page = _Page()
    with pytest.raises(RuntimeError, match="did not become ready after one reload"):
        computer_use_agent._wait_for_chromium_composer(
            page,
            expected_target_url="https://chatgpt.com/c/expected-session",
        )
    assert page.reload_calls == 1


def test_chatgpt_composer_wait_fails_when_retry_leaves_chatgpt(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import app.core.computer_use_agent as computer_use_agent
    ...
    class _Composer:
        def wait_for(self, **_kwargs: object) -> None:
            raise TimeoutError("composer missing after navigation")
    ...
    class _Page:
        ...
        def evaluate(self, _expression: str, _argument: object = None) -> dict[str, object]:
            return {"detected": False, "reason": "", "composerAvailable": False}
        ...
        def reload(self, **_kwargs: object) -> None:
            raise AssertionError("A departed ChatGPT tab must fail before reload.")
...
def test_safari_grok_receipt_watchdog_ends_without_resending(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import app.core.computer_use_agent as computer_use_agent

    target_url = "https://grok.com/c/ambiguous-submit"
    snapshots = iter(
        (
            {
                "url": target_url,
    ...
    clock = iter((0.0, 0.0, 2.0))
    monkeypatch.setattr(computer_use_agent, "CHATGPT_TURN_RECEIPT_TIMEOUT_SECONDS", 1)
    monkeypatch.setattr(computer_use_agent, "_submit_safari_prompt", submit)
    monkeypatch.setattr(
        computer_use_agent,
        "_provider_turn_snapshot",
        lambda *_args, **_kwargs: next(snapshots),
    ...
    with pytest.raises(
        computer_use_agent.AgentConnectionInterrupted,
        match="Grok did not expose its unique controller receipt within 1 seconds",
    ):
        _submit_and_wait(
            _Page(),
            "safari",
    ...
    class _Page:
        ...
        def evaluate(
            ...
            if "composerSelector" in argument:
                ...
                assert argument["composerSelector"] == _web_composer_selector("grok")
                return {"count": 1}
            if "authAction" in expression:
                return False
            api_url = str(argument["url"])
...
def test_safari_jury_provider_runtime_requires_one_exact_composer(
    monkeypatch: pytest.MonkeyPatch,
    platform: str,
    url: str,
) -> None:
    ...
    class _Page:
        ...
        def evaluate(self, expression: str, argument: object = None) -> object:
            if isinstance(argument, dict) and "composerSelector" in argument:
                self.composer_checks += 1
                assert argument["composerSelector"] == _web_composer_selector(platform)
                return {"count": 1, "url": url, "platform": platform}
            assert "authAction" in expression
            return False
    ...
    assert _verify_agent_page(page, "safari", platform, url) is True
    assert page.composer_checks == 1


def test_safari_composer_readiness_waits_for_transient_duplicate() -> None:
    import app.core.computer_use_agent as computer_use_agent

    class _Page:
        def __init__(self) -> None:
            self.counts = iter((2, 2, 1))
            self.waits: list[int] = []
    ...
    assert computer_use_agent._wait_for_unique_safari_composer(
        page,
        "claude",
    ) is True
...
def test_safari_composer_readiness_rejects_persistent_duplicates(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    import app.core.computer_use_agent as computer_use_agent
    ...
    class _Page:
        ...
        def wait_for_timeout(self, _milliseconds: int) -> None:
            raise AssertionError("A persistent duplicate must fail closed.")

    with pytest.raises(
        computer_use_agent._ComposerReadinessTimeout,
        match="Expected one Safari claude composer, found 2",
    ):
        computer_use_agent._wait_for_unique_safari_composer(
            _Page(),
            "claude",
        )
