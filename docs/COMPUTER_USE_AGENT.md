...
On macOS, Chrome Agent runs through an isolated clone of the signed-in profile. ChatGPT Edge Agent
also clones the daily signed-in profile, as Cache does, and disables CDP fallback across readiness,
source, Project, history, and task operations. On this host, a read-only Project navigation still
encounters a persistent verification page, so the task fails before prompt submission. The
evidence is recorded in [`CHATGPT_AGENT_CLOUDFLARE.md`](CHATGPT_AGENT_CLOUDFLARE.md). Other
macOS Edge Agent providers keep the
project debug profile under `local_store/agent_browser_profile/edge`. On Windows, after that
profile exists, Agent readiness, source,
...
that same profile and restart its browser after a window close. macOS ChatGPT Edge login opens the
daily Edge, while other macOS Edge Agent providers open the project debug profile over HTTP
endpoints. Recheck and tasks follow the matching profile path. Copying or launching alone does not prove
